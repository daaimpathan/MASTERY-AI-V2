...
const ActiveQuizSession = () => {
    ...
    useEffect(() => {
        const syncState = () => {
            ...
            if (storedQuiz) {
                ...
                // If quiz is gone, redirect student
                if (role === 'student') {
                    navigate('/dashboard');
                }
            }

            // 2. Sync Players (Lobby)
            const storedStudents = localStorage.getItem(QUIZ_STUDENTS_KEY);
            if (storedStudents) {
                const studentNames: string[] = JSON.parse(storedStudents);
                // Map names to Player objects (mocking score for now)
                // In a real calc, we would sum up scores from responses

                // Get responses to calc scores

                const mappedPlayers: Player[] = studentNames.map(name => {
                    // Calculate score based on correct answers
                    // This creates a dependency on knowing the questions, which we do in `quizState`
                    // For simplicity, we'll just map names first and calc scores if we have quiz data
                    return {
                        id: name, // simple ID
                        name: name,
                        score: 0 // Placeholder, updated below if possible
                    };
                });
    ...
    // Calculate Scores (Frontend Derived)
    useEffect(() => {
        if (!quizState || players.length === 0) return;
        ...
        const currentScores = JSON.stringify(players.map(p => p.score));
        const newScores = JSON.stringify(updatedPlayers.map(p => p.score));

        if (currentScores !== newScores) {
            setPlayers(updatedPlayers);
        }

    }, [quizState, players.length]); // Re-calc when quiz state (questions) or player list changes. Note: heavily simplified.
