...
def require_role(required_roles: list[UserRole]):
    ...
    async def role_checker(current_user: User = Depends(get_current_user)) -> User:
        # Robust check for role: handle both Enum and string
        # Create a set of allowed role strings
        allowed_roles_str = {str(r.value) for r in required_roles}
        ...
        if user_role_str not in allowed_roles_str:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Access denied. Required roles: {list(allowed_roles_str)}"
            )
        return current_user
