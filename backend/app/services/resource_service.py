...
class ResourceService:
    ...
    def create_resource(
        ...
        if file and resource_data.type == ResourceType.PDF:
            try:
                from app.services.google_service import google_service
                print(f"[UPLOAD] Uploading file {file.filename} to Google Drive...")
                drive_file = google_service.upload_file(file.file, file.filename)
                resource_url = drive_file.get('webViewLink')
                # Optionally store ID in file_path or leave None
                # file_path = drive_file.get('id') 
                print(f"[UPLOAD] Success. Drive Link: {resource_url}")
            except Exception as e:
                print(f"[UPLOAD] Failed to upload to Drive: {e}. Falling back to local storage.")
                # Fallback to local storage if Drive fails (e.g. auth error)
                os.makedirs("uploads/resources", exist_ok=True)
                file_extension = os.path.splitext(file.filename)[1]
                filename = f"resource_{uuid_lib.uuid4()}{file_extension}"
                local_path = f"uploads/resources/{filename}"
