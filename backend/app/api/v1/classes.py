...
def list_classes(
    ...
    # print(f"Listing classes for user: {current_user.id}, Role: {current_user.role}, Type: {type(current_user.role)}")
    ...
    if is_teacher:
        ...
        # print(f"Found {len(classes)} classes in DB for teacher")
    elif is_admin:
        classes = db.query(Class).all()
        # print(f"Found {len(classes)} classes in DB for admin")
    elif is_student:
        ...
        # print(f"User is neither teacher nor admin. Role: {current_user.role}")
        pass
    
    if not classes:
        # Only return mock if DB is empty for dev experience, or better yet, return empty list
        # print("No classes found. Returning empty list.")
        pass
